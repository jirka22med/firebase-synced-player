...
            // Nastavení hlasitosti: Používáme přímo audioPlayer.volume pro HTML5 audio
            if (loadedFirestorePlayerSettings.volume !== undefined && audioPlayer) {
                audioPlayer.volume = loadedFirestorePlayerSettings.volume;
                if (volumeSlider) volumeSlider.value = loadedFirestorePlayerSettings.volume; // Aktualizujeme i slider
            }
...
        // Nastavení hlasitosti: Používáme přímo audioPlayer.volume pro HTML5 audio
        if (savedPlayerSettings.volume !== undefined && audioPlayer) {
            audioPlayer.volume = savedPlayerSettings.volume;
            if (volumeSlider) volumeSlider.value = savedPlayerSettings.volume; // Aktualizujeme i slider
        }
...
    // Získáme aktuální hlasitost a mute stav přímo z audioPlayeru
    const currentVolume = audioPlayer ? audioPlayer.volume : 0.5;
    const isMuted = audioPlayer ? audioPlayer.muted : false;
...
        volume: currentVolume, // Ukládáme hodnotu 0-1
        muted: isMuted
    }));
...
    // Zachováno, jak jsi si přál. Vytváří strmou logaritmickou křivku.
...
    const volume = audioPlayer.volume; // HTML5 audio.volume je vždy 0-1
    const sliderValue = parseFloat(volumeSlider.value); // Hodnota slideru je také 0-1

    if (audioPlayer.muted || volume <= 0.01) { // Upraveno pro lepší citlivost u nuly
        muteButton.textContent = '🔇';
        volumeValueElement.textContent = '0';
    } else {
        // Zde počítáme procenta ze skutečné HTML5 hlasitosti (0-1)
        volumeValueElement.textContent = Math.round(volume * 100);
        if (volume <= 0.2) muteButton.textContent = '🔈';
        else if (volume <= 0.5) muteButton.textContent = '🔉';
        else muteButton.textContent = '🔊'; // Vše nad 50% je plná hlasitost ikona
    }
}
...
    }
    ...
    // *** DIAGNOSTIKA A OPRAVA: Zastavení a vynulování přehrávače před načtením nové skladby ***
    audioPlayer.pause(); // Zastaví aktuálně hrající skladbu
    audioPlayer.currentTime = 0; // Vynuluje aktuální pozici (vyčistí buffer)
    console.log("DIAGNOSTIKA: Přehrávač pozastaven a čas vynulován před načtením nové skladby.");
    // *** KONEC DIAGNOSTIKA A OPRAVA ***

    audioSource.src = track.src;
    trackTitleElement.textContent = track.title; // Aktualizace názvu skladby okamžitě
    audioPlayer.load(); // Načte nový audio soubor

    // Použijeme Promise, abychom počkali na to, než bude skladba připravena k přehrávání
    const playPromise = new Promise((resolve, reject) => {
        const handleCanPlayThrough = () => {
            console.log("DIAGNOSTIKA: Skladba připravena k přehrávání (canplaythrough).");
        ...
        const handleError = (e) => {
            console.error("DIAGNOSTIKA: Chyba při načítání mediálních dat:", e);
            audioPlayer.removeEventListener('canplaythrough', handleCanPlayThrough);
            audioPlayer.removeEventListener('error', handleError);
            reject(e);
        };
        ...
        // Přidáme timeout pro případ, že se canplaythrough nikdy nespustí (např. špatná URL nebo zpoždění)
        setTimeout(() => {
            if (audioPlayer.paused && audioPlayer.readyState < 3) { // HAVEDATA = 2, HAVE_FUTURE_DATA = 3
                console.warn('DIAGNOSTIKA: Načítání skladby trvalo příliš dlouho nebo se nezdařilo načíst dostatek dat.');
                audioPlayer.removeEventListener('canplaythrough', handleCanPlayThrough);
                audioPlayer.removeEventListener('error', handleError);
                reject(new Error('Načítání skladby trvalo příliš dlouho.'));
            }
...
            await saveAudioData();
        }).catch(error => {
            console.error('playTrack: Chyba při spouštění přehrávání HTML audio elementu:', error);
            window.showNotification(`Chyba při přehrávání: ${track.title}. Možná špatná URL nebo formát.`, 'error');
            updateButtonActiveStates(false);
        });
    }).catch(error => {
        console.error('playTrack: Zablokováno chybou načítání dat:', error);
        window.showNotification(`Chyba při načítání dat pro: ${track.title}. Možná špatná URL nebo zpoždění.`, 'error');
        updateButtonActiveStates(false);
    });
}
// *** KONEC ZMĚNĚNÉ FUNKCE playTrack ***


function updateActiveTrackVisuals() {
    console.log("updateActiveTrackVisuals: Aktualizuji vizuální zvýraznění aktivní skladby.");
    if (!playlistElement || !originalTracks || originalTracks.length === 0) return;
...
    if (audioPlayer && audioSource.src && audioSource.src !== window.location.href) {
        playTrack(currentTrackIndex); // Vždy voláme playTrack pro konzistentní chování
...
// Ovládání hlasitosti přímo přes HTML5 audio.volume
if (volumeSlider && audioPlayer) volumeSlider.addEventListener('input', async (e) => {
    // Používáme logarithmicVolume pro převod slideru 0-1 na hlasitost 0-1 s logaritmickou křivkou
    audioPlayer.volume = logarithmicVolume(e.target.value);
    updateVolumeDisplayAndIcon();
    await saveAudioData();
});

// Ovládání mute přímo přes HTML5 audio.muted
if (muteButton && audioPlayer && volumeSlider) muteButton.addEventListener('click', async () => {
    console.log("Mute button clicked.");
    audioPlayer.muted = !audioPlayer.muted;
    if (audioPlayer.muted) {
        muteButton.dataset.previousVolume = volumeSlider.value; // Uložíme si aktuální polohu slideru
        volumeSlider.value = 0; // Posuneme slider na 0 pro vizuální zobrazení
    } else {
        const prevSliderVol = muteButton.dataset.previousVolume || '0.5'; // Výchozí na 0.5 (50%)
        volumeSlider.value = prevSliderVol; // Obnovíme polohu slideru
        audioPlayer.volume = logarithmicVolume(prevSliderVol); // Nastavíme hlasitost
    }
    updateVolumeDisplayAndIcon();
    await saveAudioData();
});
...
if (audioPlayer) {
    // listener na 'volumechange' je v pořádku, ten se spouští, když se změní audioPlayer.volume
...
        case 'KeyA':
            if (volumeSlider) {
                // Snížení hlasitosti (logaritmicky)
                const currentRawVolume = parseFloat(volumeSlider.value);
                const newRawVolume = Math.max(0, currentRawVolume - 0.05);
                volumeSlider.value = newRawVolume;
                audioPlayer.volume = logarithmicVolume(newRawVolume);
                updateVolumeDisplayAndIcon();
                await saveAudioData();
            }
            break;
        case 'KeyD':
            if (volumeSlider) {
                // Zvýšení hlasitosti (logaritmicky)
                const currentRawVolume = parseFloat(volumeSlider.value);
                const newRawVolume = Math.min(1, currentRawVolume + 0.05); // Max 1 pro HTML5 audio
                volumeSlider.value = newRawVolume;
                audioPlayer.volume = logarithmicVolume(newRawVolume);
                updateVolumeDisplayAndIcon();
                await saveAudioData();
...
            const item = document.createElement('div');
            item.className = 'playlist-item';
            item.dataset.originalSrc = originalTracks.find(t => t.title === title)?.src; // Zajištění, že src existuje

            if (currentTrackIndex === originalTracks.findIndex(ot => ot.title === title) && audioPlayer && !audioPlayer.paused) {
                item.classList.add('active');
            }
...
    // Nastavení prvního tracku po načtení dat
    if (currentPlaylist.length > 0 && audioPlayer && audioSource && trackTitleElement) {
...
    // Aplikace uložené hlasitosti po načtení dat
    if (audioPlayer.volume !== undefined && volumeSlider) {
        volumeSlider.value = audioPlayer.volume; // Nastavíme slider na načtenou hlasitost
    }
    if (audioPlayer.muted !== undefined) {
        audioPlayer.muted = audioPlayer.muted; // Nastavíme ztlumení podle uloženého stavu
    }
    updateVolumeDisplayAndIcon(); // Aktualizovat zobrazení a ikonu
